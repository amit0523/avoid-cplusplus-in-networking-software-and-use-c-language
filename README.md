...
5. I had worked on a networking project in C++ but actually that was not true C++. They just wrote C code in C++ classes. So, they just used the class feature of C++, the rest was C code. So, then why to use C++ just to write C code in C++ classes? Why not simply use C language and write C code?
